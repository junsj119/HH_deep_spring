...
import com.sparta.week04.Week04Application;
import com.sparta.week04.controller.ProductRestController;
...
@WebMvcTest(
        controllers = {UserController.class, ProductRestController.class},      //컨트롤러 별로 나눠도 된다.
        excludeFilters = {
...
class UserProductMvcTest {
    ...
    // 가짜 빈     if 컨트롤러에서 서비스로 요청했는데 그 처리가 문제가 됐다고 가정하는데, 전체적인 테스트에 이상을 주지 않게 하기 위해서
    // 전체적인 테스트에 이상이 없도록 디펜던시를 끊는?
    @MockBean
...
// when - then
        //get메소드로 /user/login을 실행한다.
        mvc.perform(get("/user/login"))
                .andExpect(status().isOk()) // 200이 나와야함
                .andExpect(view().name("login"))    // view name이 다르면 error
                .andDo(print());
    }
...
// when - then
        //post로 /user/signup에 보내겠다. parameter을 실어서
        mvc.perform(post("/user/signup").params(signupRequestForm))
                .andExpect(status().is3xxRedirection())     //isOk()마냥 확인하는 거
...
// given
        ...
        String postInfo = objectMapper.writeValueAsString(requestDto);  //JSON형태를 담은 String타입
